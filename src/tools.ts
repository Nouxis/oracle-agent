...
export function createTools(store: VectorStore, onInputRequired?: InputRequiredCallback) {
  const lookupProtocol = tool({
    ...
    description:
      "Search Nouxis protocol technical details: instructions, PDAs, error codes, " +
  ...
  const lookupSdk = tool({
    name: "lookup_sdk",
    description:
      "Search Nouxis SDK usage: @Nouxis-ai/sdk — instructions, PDA helpers, account fetchers, " +
    ...
    parameters: z.object({
      query: z
        ...
        .describe(
          ...
          "'DelegationBuilder usage', 'NouX Express middleware setup'",
        ),
    }),
    execute: async ({ query }) => {
  ...
  const lookupFaq = tool({
    name: "lookup_faq",
    description:
      "Search frequently asked questions about Nouxis: general overview, agents & NFTs, " +
    ...
    parameters: z.object({
      query: z
        .string()
        .describe(
          "Natural language search query, e.g. 'what is Nouxis', " +
          "'who founded Nouxis', 'how does NouX payment work'",
        ),
    }),
    execute: async ({ query }) => {
