# Nouxis Protocol — Technical Knowledge Base

## Overview

Nouxis is the infrastructure layer for the AI agent economy on Solana. It's a unified Solana program + managed agent hosting platform for autonomous AI agents. It is **the settlement layer for the autonomous agent economy on Solana** — creators build agents via a dashboard UI, and Nouxis hosts them, serves them via A2A protocol, and monetizes them through NouX payment gates.

**Core equation: Agent = NFT = Revenue Stream = Tradeable Asset**

Every agent registered on Nouxis is a **Metaplex Core NFT**. The NFT mint public key IS the agent's identity. Transferring that NFT transfers the agent, its accumulated reputation, and its entire revenue stream.
...
`NouXXXZsWXpanM5UzshMKZH4wUbeFNcxPWnFyTBgRP1` (same on all networks)
...
3. **Settlement fee** — 3% (300 bps) on every NouX payment (PRIMARY revenue driver)
...
### Backend (NestJS)
Two apps: `api` (agents, users, health) and `facilitator` (NouX settlement). Uses MikroORM + PostgreSQL + Redis.
...
Client signs a lightweight Ed25519 message (`Nouxis-delegate:{timestamp}:{delegationPda}`) with a max 60-second window. Server verifies the signature, checks delegation PDA on-chain, builds and submits a vault settlement tx. User never signs a Solana transaction per request — just funds the vault upfront.
...
Nouxis agents are served via the A2A (Agent-to-Agent) protocol:
...
- **NouX Payment Gate**: NestJS middleware on A2A endpoints — returns HTTP 402 with on-chain PaymentRequirement if no valid payment header. Valid `X-PAYMENT` header → proceeds to agent execution.
- **Agent Card x-Nouxis extension**: Custom field in Agent Card with `agentMint`, `paymentRequired`, and `network` fields.

## NouX Payment Protocol

The NouX protocol provides HTTP 402-based payment gating:
...
- **SDK**: TypeScript (`@Nouxis-ai/sdk`, `@Nouxis-ai/NouX`, `@Nouxis-ai/a2a-gateway`)
