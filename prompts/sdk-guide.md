# @Nouxis-ai/sdk — Developer Reference

The Nouxis SDK is a single TypeScript package (`@Nouxis-ai/sdk`) for building on the Nouxis Solana protocol. It provides instruction builders, PDA derivation, account fetchers, event parsing, error handling, and a zero-dependency REST API client.
...
import { registerAgent, fetchAgentIdentity, findAgentPda } from "@Nouxis-ai/sdk";

// Settlement — verify_and_settle + close_receipt instruction builders
import { buildVerifyAndSettleIx, buildCloseReceiptIx } from "@Nouxis-ai/sdk/settlement";

// API Client — zero-dependency REST client (uses native fetch)
import { NouxisApiClient } from "@Nouxis-ai/sdk/api";

// IDL — Anchor IDL types and JSON
import { IDL } from "@Nouxis-ai/sdk/idl";
```
...
import { IDL, type Nouxis } from "@Nouxis-ai/sdk/idl";
import { NETWORKS } from "@Nouxis-ai/sdk";

const connection = new Connection(clusterApiUrl("devnet"));
const provider = new AnchorProvider(connection, wallet, {});
const net = NETWORKS.devnet;
const program = new Program<Nouxis>(IDL, net.programId, provider);
```
...
```typescript
import { Nouxis_PROGRAM_ID, NETWORKS, DEFAULT_REGISTRATION_FEE, DEFAULT_SETTLEMENT_FEE_BPS } from "@Nouxis-ai/sdk";

Nouxis_PROGRAM_ID  // "NouXXXZsWXpanM5UzshMKZH4wUbeFNcxPWnFyTBgRP1"
...
```typescript
import { ServiceType, PaymentScheme, SettlementMode } from "@Nouxis-ai/sdk";
...
```typescript
import { addService, updateService, removeService, ServiceType } from "@Nouxis-ai/sdk";
...
```typescript
import { giveFeedback, revokeFeedback } from "@Nouxis-ai/sdk";
...
```typescript
import { validationRequest, validationResponse } from "@Nouxis-ai/sdk";
...
```typescript
import { DelegationBuilder, revokeDelegation } from "@Nouxis-ai/sdk";
...
```typescript
import { createDelegation } from "@Nouxis-ai/sdk";
...
```typescript
import { buildVerifyAndSettleIx, buildCloseReceiptIx } from "@Nouxis-ai/sdk/settlement";
import { SettlementMode, randomPaymentId, findAgentPda, findPaymentReqPda } from "@Nouxis-ai/sdk";
...
```typescript
import { parseEventsFromTx, parseEventsFromLogs, findEvent } from "@Nouxis-ai/sdk";
...
```typescript
import { parseNouxisError } from "@Nouxis-ai/sdk";

try {
  await program.methods.registerAgent(...).rpc();
} catch (err) {
  const NouxisError = parseNouxisError(err);
  if (NouxisError) {
    console.log(NouxisError.code);    // e.g. 6012
    console.log(NouxisError.name);    // e.g. "Unauthorized"
    console.log(NouxisError.message); // human-readable explanation
  }
}
```
...
Zero-dependency HTTP client for the Nouxis REST API. Uses native `fetch`.

```typescript
import { NouxisApiClient } from "@Nouxis-ai/sdk/api";

// Connect to network
const api = new NouxisApiClient();                                  // mainnet default
const api = new NouxisApiClient({ network: "devnet" });            // devnet
const api = new NouxisApiClient({ baseUrl: "http://localhost:3001" }); // custom
